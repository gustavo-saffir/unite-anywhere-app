...
create type public.user_position as enum ('pastor', 'lider', 'discipulo');
...
create table public.profiles (
  ...
  church_denomination text not null,
  position user_position not null default 'discipulo',
...
as $$
begin
  insert into public.profiles (id, full_name, church_denomination, position)
  values (
    new.id,
    coalesce(new.raw_user_meta_data->>'full_name', ''),
    coalesce(new.raw_user_meta_data->>'church_denomination', ''),
    coalesce((new.raw_user_meta_data->>'position')::user_position, 'discipulo')
  );
